...
public class GraphExplosionFeasiblePathAlgorithm<State, Parameter, Context>
  ...
  private DirectedPseudograph<Configuration<State, EEFSMContext<Context>>, TransitionWrapper>
      explodedEEFSM = new DirectedPseudograph<>(null);
  ...
  public GraphExplosionFeasiblePathAlgorithm(EEFSM<State, Parameter, Context> eefsm) {
    ...
    explode(eefsm);
    logger.debug(
        "Exploding EEFSM took {}. Exploded EEFSM contains {} nodes and {} transitions.",
        sw,
        explodedEEFSM.vertexSet().size(),
   ...
   */
  public EEFSMPath<State, Parameter, Context> getPath(
      Configuration<State, EEFSMContext<Context>> config, State tgt) {
   ...
   *     leads to a different terminal configuration that contains the target state.
   */
  @Override
   ...
   */
  @Override
  public List<EEFSMPath<State, Parameter, Context>> getPaths(
      Configuration<State, EEFSMContext<Context>> config, State tgt) {
  ...
  /**
   * Returns a cached set of feasible path between the current state of the EEFSM and any target. If
   * a path is feasible depends on the semantics of the underlying efsm implementation. The
   * algorithm will assume the current configuration of the efsm as start configuration.
   *
   * @return A {@link SingleSourceShortestPath} instance containing all feasible path from the
   *     current EEFSM configuration
   */
  ...
  /**
   * Returns a cached set of feasible path between the given state of the efsm and any target. If a
   * path is feasible depends on the semantics of the underlying efsm implementation. The algorithm
   * will assume given configuration of the efsm as start configuration.
   *
   * <p>Note: The implementation of this interface has to decide which paths to returen, e.g., all
   * feasible path or a subset
   *
   * @param config The configuration from which a path should be calculated The source configuration
   *     for which a single source shortest path set should be collected
   * @return A {@link SingleSourceShortestPath} instance containing all feasible path from the given
   *     EEFSM configuration
   */
  public SingleSourceShortestPath getSingleSourceShortestPath(
      Configuration<State, EEFSMContext<Context>> config) {
    // the given configuration might not exist in the exploded graph which means there is not even a
    // single path to it.
    if (!explodedEEFSM.containsVertex(config)) {
      return null;
  ...
  /**
   * Returns a list of all configuration containing the given target state which are reachable by
   * the given {@link ShortestPathAlgorithm.SingleSourcePaths}
   *
   * @param tgt
   * @param baseSSSP
   * @return
   */
  private List<EEFSMPath<State, Parameter, Context>> getPathsForSSSP(
      State tgt,
      ShortestPathAlgorithm.SingleSourcePaths<
              Configuration<State, EEFSMContext<Context>>, TransitionWrapper>
          baseSSSP) {
    ...
    logger.trace(
        "Computing paths from {} to {} resulted in {} paths and took {}",
        baseSSSP.getSourceVertex(),
        tgt,
        res.size(),
        sw);
  ...
  private EEFSMPath<State, Parameter, Context> toEFSMPath(
      GraphPath<Configuration<State, EEFSMContext<Context>>, TransitionWrapper> path) {
  ...
  public Collection<Configuration<State, EEFSMContext<Context>>> getFeasibleConfigsForState(
      State state) {
  ...
  /**
   * Single source shortest path implementation for {@link GraphExplosionFeasiblePathAlgorithm}.
   * Caches all shortest path for a given source node and can be useful if multiple have to be
   * queried without re-computation.
   */
  public class SingleSourceShortestPath
      implements IFeasiblePathAlgo.SingleSourceShortestPath<
          State, Parameter, EEFSMContext<Context>, ETransition<State, Parameter, Context>> {

    private final ShortestPathAlgorithm.SingleSourcePaths<
            Configuration<State, EEFSMContext<Context>>, TransitionWrapper>
        baseSSSP;
    ...
    private final LoadingCache<State, List<EEFSMPath<State, Parameter, Context>>> tgtToPaths =
        CacheBuilder.newBuilder()
            .build(
                new CacheLoader<State, List<EEFSMPath<State, Parameter, Context>>>() {
                  @Override
                  public List<EEFSMPath<State, Parameter, Context>> load(State tgt)
                      throws NoPathForTargetException {
                    final List<EEFSMPath<State, Parameter, Context>> pathsForSSSP =
                        getPathsForSSSP(tgt, baseSSSP);

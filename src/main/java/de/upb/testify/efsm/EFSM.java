...
public class EFSM<State, Parameter, Context extends IEFSMContext<Context>,
  ...
  protected EFSM(Graph<State, Transition> baseGraph, State initialState, Context initalContext) {
    ...
    final DirectedPseudograph<State, Transition> tmp = new DirectedPseudograph<>((src, tgt) -> {
      throw new IllegalStateException(
          "Edges should not be added without a transition object. We cannot infer a specific transition object due to generics.");
    });
  ...
  /**
   * This will track changes to the base graph but not configuration changes to the efsm. Also, no property listeners are
   * copied. Shouldn't be that expensive since only the context and state change
   *
   * @return
   */
